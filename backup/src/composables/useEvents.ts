...
export default function useEvents(getYearRef: any = null, getMonthRef: any = null, getDayRef: any = null, getToggleRef: any = null) {
  ...
  const setEventsInBetween = async () => {
    const start = startDateRef.value ? startDateRef.value.split('-') : [""];
    const end = endDateRef.value ? endDateRef.value.split('-') : [""];
    getEventsInBetween.value = await fetchEventsInBetween(start, end)
  }
